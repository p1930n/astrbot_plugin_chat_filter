...
FAILED_REASON_ACTION_CALL_TIMEOUT = "onebot_action_call_timeout"
DEFAULT_FORWARD_NODE_NAME = "Chat Filter"
ONEBOT_ACTION_CALL_TIMEOUT_SECONDS = 10.0
...
class PlatformActionLogger(Protocol):
    def error(self, message: str, *args: object) -> None:
        ...


class _NoopPlatformActionLogger:
    def error(self, message: str, *args: object) -> None:
        _ = message, args
...
class OneBotV11PlatformActions:
    def __init__(
        ...
        *,
        ...
        action_timeout_seconds: float = ONEBOT_ACTION_CALL_TIMEOUT_SECONDS,
    ) -> None:
        self._action_client = action_client
        self._logger = logger or _NoopPlatformActionLogger()
    ...
    async def _call_action(self, action: str, **params: Any) -> PlatformActionResult:
        ...
        try:
            await asyncio.wait_for(
                self._action_client.call_action(action, **params),
            ...
            return PlatformActionResult.failed(FAILED_REASON_ACTION_CALL_TIMEOUT)
        except Exception as exc:
            self._logger.error(
                "Chat Filter OneBot action call failed: action=%s error_type=%s",
                action,
                type(exc).__name__,
            )
...
def format_platform_probe(
    ...
    return "\n".join(
        [
            ...
            f"{_capability_state(capabilities.send_forward_message)}",
